...
var modals = function modals(state) {
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var fieldsToValidate = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : null;
    ...
    trigger.forEach(function (t) {
      t.addEventListener('click', function (e) {
        var validateErrorMsg = document.querySelector('#validate-error-msg');
        ...
        if (state && fieldsToValidate) {
          ...
          fieldsToValidate.forEach(function (item) {
            if (!state[item]) {
              errors.push(item); // console.log('validation error: ', item);
            }
          });
  ...
  bindModal('.popup_calc_profile_button', '.popup_calc_end', '.popup_calc_end_close', false, state, ['type', 'profile']); // showModalByTime('.popup', 60000);
};
